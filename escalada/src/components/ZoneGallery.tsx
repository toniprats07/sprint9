...
// Definición de la interfaz Zone
interface Zone {
  id: string;
  title: string;
  description: string;
...
const ZoneGallery: React.FC = () => {
  const [zones, setZones] = useState<Zone[]>([]); // Estado para almacenar las zonas
  const [selectedZone, setSelectedZone] = useState<Zone | null>(null); // Estado para la zona seleccionada

  useEffect(() => {
    // Función para obtener las zonas desde Firestore
    const fetchZones = async () => {
      const db = getFirestore();
      const zonesCollection = collection(db, 'zonasPopulares');
      const zonesSnapshot = await getDocs(zonesCollection);
      const zonesData = zonesSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })) as Zone[];
      setZones(zonesData);
    };

    fetchZones(); // Llamar a la función para obtener las zonas
  }, []);

  const handleModalOpen = (zone: Zone) => {
    setSelectedZone(zone); // Establecer la zona seleccionada
  };

  const handleModalClose = () => {
    setSelectedZone(null); // Limpiar la zona seleccionada
  };
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-72 mb-20">
      {zones.map((zone) => (
        <div className="card h-auto bg-base-100 shadow sm:max-w-lg md:w-64 lg:max-w-xl" key={zone.id}>
          <figure>
            <img className="h-52 sm:h-72 w-full object-cover" src={zone.imageUrl} alt={zone.title} />
          </figure>
          ...
          <div className="card-body p-3 items-center">
            <h2 className="card-title">{zone.title}</h2>
